...
export async function drawTemplateOnTile(tileBlob, tileCoords) {
  const [tX, tY] = tileCoords, S = state, T = TILE_SIZE, D = DRAW_MULT;
  ...
  const cX = M.max(0, tWSX - cTWSX) * D, cY = M.max(0, tWSY - cTWSY) * D;

  if (!sourceCanvas) {
    sourceCanvas = document.createElement('canvas');
    sourceCanvas.width = S.w; sourceCanvas.height = S.h;
  ...
  if (dW > 0 && dH > 0) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      if (((i / 4) % w) % D !== 1 || M.floor((i / 4) / w) % D !== 1) d[i + 3] = 0;
    }
